...
test("buildLlmHandoff returns a neutral data context packet", () => {
  ...
  assert.match(handoff.promptText, /observational data only/);
  assert.match(handoff.promptText, /Source freshness and coverage/);
  assert.match(handoff.promptText, /Current app-observed state/);
  ...
  assert.match(handoff.promptText, /Weekly observed data/);
  assert.match(handoff.promptText, /Nutrition/);
  assert.match(handoff.promptText, /Activity/);
  assert.match(handoff.promptText, /Training availability status: Train/);
  assert.match(handoff.promptText, /Training-load bottleneck data: 4 lifts \/ 54 sets in 7 days; 4 lifts \/ 54 sets Mon-Sun/);
  assert.match(handoff.promptText, /Load flags: high training load yes; recent load spike no; consecutive lifting days 2/);
  assert.match(handoff.promptText, /Weekly goal pressure data: days left 3; lifts remaining for weekly goal 0/);
  ...
  assert.match(handoff.promptText, /Recent high-resolution data/);
  ...
  assert.match(handoff.promptText, /App recommendation candidates as evidence: training\/high\/high: Keep intensity moderate/);
  ...
  assert.match(handoff.promptText, /Completed Mon 2026-03-30: UPPER A; classified Upper/);
  assert.match(handoff.promptText, /Today Sat 2026-04-04: actual workout none logged yet/);
  ...
  assert.doesNotMatch(handoff.promptText, /HealthMax deterministic output/);
  assert.doesNotMatch(handoff.promptText, /Training target/);
  assert.doesNotMatch(handoff.promptText, /Next training target/);
  assert.doesNotMatch(handoff.promptText, /Intensity intent/);
  assert.doesNotMatch(handoff.promptText, /Intensity cue/);
  assert.doesNotMatch(handoff.promptText, /Session anchors/);
  assert.doesNotMatch(handoff.promptText, /Calorie recommendation field/);
  assert.doesNotMatch(handoff.promptText, /rationale/i);
  assert.doesNotMatch(handoff.promptText, /guardrail/i);
  assert.doesNotMatch(handoff.promptText, /intent Push/);
  assert.doesNotMatch(handoff.promptText, /intent Maintain/);
  assert.doesNotMatch(handoff.promptText, /intent Back off/);
  assert.doesNotMatch(handoff.promptText, /recommended/i);
