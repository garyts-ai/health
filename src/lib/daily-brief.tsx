...
export function buildDiscordSummaryText(summary: DailySummary) {
  ...
  const latestLiftDetail =
    handoff.latestLiftFocus.length > 0
      ? handoff.latestLiftFocus.slice(0, 4).join(", ")
      : `Upper ${summary.trainingLoad.upperBodyDaysSince ?? "--"}d | Lower ${summary.trainingLoad.lowerBodyDaysSince ?? "--"}d`;
  ...
  return [
    ...
    `Weekly muscle focus: ${weeklyFocusText} | ${summary.trainingLoad.hevyWorkoutCount7d} workouts this week`,
    `Weight ${formatPounds(kilogramsToPounds(summary.readiness.bodyWeightKg))} | ${handoff.bodyWeightTrendLabel}`,
    `Latest lift: ${summary.trainingLoad.hevyLastWorkoutTitle ?? "None logged"} | ${latestLiftDetail}`,
    `**Ask the LLM:** ${handoff.llmQuestion}`,
    "_Image attached below is optimized for quick review and fresh LLM judgment._",
  ].join("\n");
}
